...
func TestServer_Advertises_Common_Cipher_SE(t *testing.T) {
	const requiredSuite = tls.TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384
	st := newServerTester(t, nil, func(c *tls.Config) {
...
func TestConfigureServer(t *testing.T) {
	tests := []struct {
		...
		{
			name: "just the http2se cipher suite",
			tlsConfig: &tls.Config{
				CipherSuites: []uint16{tls.TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384},
			},
		},
...
func TestServerHandleCustomConn(t *testing.T) {
	...
	fakeConnState := tls.ConnectionState{
		...
		CipherSuite: cipher_TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384,
	}
